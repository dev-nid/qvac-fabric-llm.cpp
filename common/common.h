...
struct common_params_speculative {
    ...
    bool     dflash_tree             = false; // enable multi-seq tree-shaped verify
    int32_t  dflash_tree_budget      = 0;     // tree node budget; 0 = default shape (= block_size)
    bool     dflash_tree_best_first  = false; // expand tree by cumulative log-prob (heap) instead of round-robin

    // when true, the target context is constructed with cparams.dflash_inline_encoder
    // set, and the speculative driver calls llama_dflash_bind_encoder() to plumb
    ...
    // GatedDeltaNet history kernel. When true, the GDN op writes per-token
    // recurrent state to a persistent per-layer buffer; on partial acceptance
    // the speculative driver replaces the checkpoint+re-verify path with a
    // fixup graph that selects state[K-1] for each GDN layer (with standard
    // seq_rm for full-attention layers). CUDA only. Target-only; cleared on
    // the draft cparams in the spec driver.
    bool     dflash_gdn_history      = false;

    // GDN history persistent buffer dtype. When true, allocate gdn_history[il]
    // as F16 (halves the per-layer footprint). Auto-defaults true when
    // --dflash-tree && --dflash-gdn-history are both set; default false in
    // chain mode.
