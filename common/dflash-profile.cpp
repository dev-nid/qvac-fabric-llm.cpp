...
void memory_profiler::snapshot(const std::string & label,
    ...
    if (draft_ctx != nullptr) {
        llama_dflash_memory_buckets b{};
        llama_dflash_memory_breakdown(draft_ctx, &b);
        // Draft owns the side store + top-K read-back buffers.
        s.side_store_K_bytes      += b.side_store_K_total;
        s.side_store_V_bytes      += b.side_store_V_total;
        s.draft_topk_bytes        += b.draft_topk_bytes;
        s.draft_topk_argmax_bytes += b.draft_topk_argmax_bytes;
        s.ctx_capacity             = b.ctx_capacity;
        s.ctx_filled               = b.ctx_filled;
        s.n_layers                 = b.n_layers;
    }
    if (target_ctx != nullptr) {
        llama_dflash_memory_buckets b{};
        llama_dflash_memory_breakdown(target_ctx, &b);
        // Target owns the captured-features + per-layer capture-staging
        // buffers (the side that runs the capture-emitting decoder graph).
        s.captured_features_bytes += b.captured_features_bytes;
        s.capture_staging_bytes   += b.capture_staging_bytes;
    }
