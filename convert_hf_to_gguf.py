...
class DFlashModel(Qwen3Model):
    ...
    def set_gguf_parameters(self):
        ...
        if target_layer_ids is None:
            # Fall back to the heuristic from dflash/dflash/model.py:
            #   - single layer -> [n_target/2]
            #   - multi -> evenly spaced across [1, n_target-3]
    ...
    def modify_tensors(self, data_torch, name, bid):
        ...
        # target model and keep them frozen during training. Most published
        # DFlash checkpoints therefore do not include these tensors; if they
        # *do* (e.g. a community checkpoint that bundled them), we drop them
        # so the resulting GGUF is paper-faithful and forces the inference
        # engine to bind them from the target via llama_dflash_bind_target.
        # (The C++ loader marks tok_embd/output as TENSOR_NOT_REQUIRED so a
        # missing-tensor GGUF loads cleanly.)
