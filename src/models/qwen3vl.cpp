...
llm_build_qwen3vl::llm_build_qwen3vl(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // DFlash: tee out this layer's hidden state (post-deepstack so the
        // captured features match what the published Qwen3-VL-DFlash
        // checkpoints were trained on). No-op when the context isn't being
        // used as a DFlash target.
