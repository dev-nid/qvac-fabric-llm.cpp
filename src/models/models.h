...
//   pos_idx           : I64 [n_new_tokens]   (= side-store column indices to
//                                              write to; commonly =
//                                              [write_offset..write_offset+n_new-1]
//                                              but can be any permutation)
//
// Outputs (no `t_logits` / `t_embd`): per-layer ggml_set_rows nodes that
// scatter K_new and V_new into ctx_K[il] / ctx_V[il] at the columns
// indicated by pos_idx. Caller is responsible for bumping `ctx_filled` by
// `n_new_tokens` after compute.
//
// The graph topology depends ONLY on `n_new_tokens` (= the encoder
// `n_tokens` derived from the stub ubatch in `dflash_extend`); the
// destination columns are runtime inputs via `pos_idx`. This lets
// `dflash_extend` cache the built graph by `n_new` and skip
// `res->reset()` + rebuild + `ggml_backend_sched_alloc_graph` on the
// common path where consecutive blocks have the same `n_new`
// (e.g. all blocks with the same accept count).
