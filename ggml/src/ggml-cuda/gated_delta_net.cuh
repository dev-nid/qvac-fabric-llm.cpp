...
// DFlash Phase 4: same kernel as ggml_cuda_op_gated_delta_net, but the dst
// tensor packs an additional per-token state-history region of shape
// [S_v, S_v, H_v, n_tokens, n_seqs] after the attn-output and final-state
...
// DFlash Phase 4 fixup: strided copy that selects
...
// DFlash Phase 4 conv-state fixup: picks `conv_state_rows` consecutive
// rows of conv_history along dim 0 starting at offset (k_index + 1),
// and writes them into a [conv_state_rows, conv_channels, n_seqs]
